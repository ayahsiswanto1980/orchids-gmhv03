...
import { 
  ...
  Share2, Image, Facebook, Instagram, Twitter, Youtube, Layout
} from 'lucide-react';
...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (sectionData) setSectionContent(sectionData);
  ...
  const sectionLabels: Record<string, string> = {
    promotions: 'Penawaran Spesial / Promo',
    rooms: 'Akomodasi / Pilihan Kamar',
    facilities: 'Fasilitas Hotel',
    gallery: 'Visual Experience / Galeri',
    services: 'Layanan Unggulan',
    testimonials: 'Testimonial / Ulasan',
    location: 'Lokasi & Peta',
    partners: 'Our Partners / Partner Kami'
  };

  const handleSectionChange = (key: string, field: 'title' | 'subtitle', value: string) => {
    setSectionContent(prev => prev.map(item => 
      ...
      <Tabs defaultValue="general" className="space-y-6">
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-6 h-auto">
          ...
          <TabsTrigger value="sections" className="flex items-center gap-2 py-3">
            ...
            <span className="hidden sm:inline">Judul Section</span>
          </TabsTrigger>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="font-display">Judul & Subjudul Section</CardTitle>
              </div>
              <CardDescription>Sesuaikan judul dan deskripsi singkat pada setiap bagian halaman publik</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 gap-6">
                  {sectionContent.map((item) => (
                    <div key={item.section_key} className="space-y-4 p-4 border rounded-lg bg-muted/30">
                      <div className="flex items-center justify-between">
                        <h3 className="font-semibold text-gold">{sectionLabels[item.section_key] || item.section_key.replace('_', ' ')}</h3>
                      </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input 
                          value={item.title} 
                          onChange={(e) => handleSectionChange(item.section_key, 'title', e.target.value)}
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea 
                          value={item.subtitle} 
                          onChange={(e) => handleSectionChange(item.section_key, 'subtitle', e.target.value)}
                          placeholder="Masukkan subjudul section..."
                          rows={3}
                        />
                      </div>
                    </div>
