...
import { Bed, Building2, Wrench, Star, Tag, Image as ImageIcon } from 'lucide-react';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true);
        const [roomsRes, facilitiesRes, servicesRes, reviewsRes, promoRes, galleryRes] = await Promise.all([
  ...
  const statCards = [
    { title: 'Promo', value: stats.promotions, icon: Tag, color: 'text-orange-500', bg: 'bg-orange-500/10' },
    ...
    { title: 'Galeri', value: stats.gallery, icon: ImageIcon, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <a href="/admin/promotions" className="p-4 rounded-lg border border-border hover:border-gold/30 hover:bg-gold/5 transition-all group">
              <Tag className="w-8 h-8 text-gold mb-2 group-hover:scale-110 transition-transform" />
              <h3 className="font-semibold">Kelola Promo</h3>
              <p className="text-sm text-muted-foreground">Update penawaran spesial</p>
            </a>
            ...
            <a href="/admin/gallery" className="p-4 rounded-lg border border-border hover:border-gold/30 hover:bg-gold/5 transition-all group">
              <ImageIcon className="w-8 h-8 text-gold mb-2 group-hover:scale-110 transition-transform" />
              <h3 className="font-semibold">Kelola Galeri</h3>
              <p className="text-sm text-muted-foreground">Update foto-foto hotel</p>
            </a>
            <a href="/admin/facilities" className="p-4 rounded-lg border border-border hover:border-gold/30 hover:bg-gold/5 transition-all group">
              ...
              <p className="text-sm text-muted-foreground">Atur fasilitas premium</p>
            </a>
          </div>
        </CardContent>
      </Card>
